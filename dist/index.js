...
async function runValidation(options) {
    ...
    const retailParser = new parser_1.AngularParser(retailPath);
    const retailFiles = retailParser.parseDirectory(retailPath, extensions, true);
    console.log('Running Angular parser on restaurant...');
    const restaurantParser = new parser_1.AngularParser(restaurantPath);
...
async function runAnalysis(options) {
    ...
    const matcher = new matcher_1.FileMatcher(retailPath, restaurantPath);
    ...
    const differ = new diff_1.GitDiffer(repoRoot, options.baseCommit || null);
    ...
    const graphBuilder = new graph_1.GraphBuilder(config);
    const { nodes, edges } = graphBuilder.build(retailFiles, restaurantFiles, matchResult.matched, matchResult.retailOnly, matchResult.restaurantOnly, diffResults);
    ...
    const analyzer = new graph_1.GraphAnalyzer();
    const analysisResult = analyzer.analyze(nodes, edges);
    const reportGenerator = new report_1.ReportGenerator(differ);
