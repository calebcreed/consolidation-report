...
const analyzer_1 = require("../graph/analyzer");
class ReportGenerator {
    constructor(differ) {
        this.differ = differ;
        this.analyzer = new analyzer_1.GraphAnalyzer();
    }
    generate(result, diffResults, outputPath) {
        ...
        for (const node of result.nodes.values()) {
            ...
            if (diffResult && node.divergence?.type !== 'CLEAN') {
                ...
                let { retailDiff, restaurantDiff } = this.differ.generateUnifiedDiff(diffResult.baseContent, diffResult.retailContent, diffResult.restaurantContent, filename);
